...
4. **Bias Detection**  
   ...
   * **On the job-title side**, we deployed a **two-step matching process**:
      ...
      2. **Semantic matching** on top of that, which catches synonyms or paraphrases of the same job experience, making our extraction robust to differences in how the parser names each job.
      (es,  Parsed skill: Hotel Receptionist -> , Semantic Matcher: -> Receptionist) !!!
   ...
   For the **DRIVERSLIC** and **LANGUAGE_SKILL**, we focused on **false negatives**: candidates for whom the regex extractor found a match but the parser returned nothing. 
   
   To make these insights more accessible, we built a **Streamlit app** that lets you s into individual candidates. For each false negative, the interface displays the raw CV text on the left with regex matched segments highlighted in red and on the right shows the skills the parser actually assigned to that candidate.

   Once we reviewed and analyzed the results of our methods, we broadened our bias assessment to the four previously identified, (gender, age, geographical position, hard vs soft skills) to determine whether the parser’s errors disproportionately impact under or over represented populations.
